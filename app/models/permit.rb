class Permit < ActiveRecord::Base
  ...
  validates :start_date, timeliness: { on_or_after: lambda { Date.today } },    if: ->(f){ f.permit_type != 'daily'}
  validates :expiration_date, timeliness: { on_or_after: :start_date },         if: ->(f){ f.permit_type != 'daily'}
