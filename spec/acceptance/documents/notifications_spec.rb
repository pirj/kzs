...
feature "User receives documents notifications", %q() do
  ...
  let!(:incoming_mail) {
    document = FactoryGirl.build(:simple_document, recipient_organization: mail.sender_organization)
    FactoryGirl.create(:mail, document: document)
  }
  ...
  let(:reply_path) { reply_documents_official_mail_path(incoming_mail) }
  ...
  describe 'notifications after create replying mail', js: true do
    context 'creating new mail' do
      let(:replying_mail) { Documents::OfficialMail.last }
      let(:creator) { replying_mail.creator }

      # оповещения создаются по-настоящему в контроллерах, никаких колбеков в моделях
      # поэтому нужно зайти под главным и протолкнуть письмо далее по цепочке статусов
      background do
        expect(incoming_mail.recipient_organization).to eq approver.organization

        visit reply_path
        sign_in_with approver.email
        skip_welcome
        expect(current_path).to eq reply_path

        select_from_chosen 'Контрольное лицо'
        fill_in 'Тема', with: 'Тема ответного письма'
      ...
      context 'approver of the reply' do
        let(:user) { replying_mail.approver }

        scenario 'approver and creator is different people' do
